...
// Belum Clockout
// "SELECT * FROM users JOIN absensi ON users.user_id = absensi.user_id WHERE tgl != '$tgl' GROUP BY nama_lengkap ORDER BY users.user_id";
...
$user = new Users();
if (isset($_POST["edit"])) {
    $user->set_user_edit_data($_POST["user_id"], $_POST["nama"], $_POST["role"], $_POST["password"]);

    $user_id = $user->get_user_id();
    $nama_lengkap = $user->get_user_nama();
    $role = $user->get_user_role();
    $password = $user->get_user_password();
    $sql = "UPDATE users SET nama_lengkap = '$nama_lengkap', role ='$role',password ='$password' WHERE user_id = $user_id";
    $result = $db->query($sql);
    
    if($result){
        if($db->affected_rows > 0){
            header("location:./edit-data.php?message=Data Berhasil di Ubah!!");
        } else {
            header("location:./edit-data.php?message=Data Gagal di Ubah");
        }
    }
        ...
        <div class="side-navbar" style="background-color: var(--bs-dark-bg-subtle);">
            <nav>
                <ul>
                    ...
                    <li class="nav-item"><a href="#" class="fw-semibold"><i class="fa-solid fa-user-gear"></i>&nbsp;&nbsp;Edit Data</a></li>
                </ul>
            </nav>
        </div>
    ...
    background-color:var(--bs-body-bg) ;" type="button" data-bs-toggle="dropdown" aria-expanded="false">
        ...
        -moz-box-shadow: 10px 14px 0px 0px rgba(0,0,0,0.53); background-color:var(--bs-light-bg-subtle);color:var(--bs-light-text);border:1px solid var(--bs-light-border-subtle);">
                        ...
                        <form class="d-flex flex-column gap-3" action="" method="POST">
                            <p class="mb-1 ">NIP</p>
                            <input class="input-edit rounded-2" name="user_id" type="text">
                            <p class="mb-1">Nama</p>
                            <input class="input-edit rounded-2" name="nama" type="text">
                            <p class="mb-1">Jabatan</p>
                            <input class="input-edit rounded-2" name="role" type="text">
                            <p class="mb-1">Password</p>
                            <input class="input-edit rounded-2" name="password" type="text">
                            <button class="btn btn-primary fw-semibold" type="submit" name="edit">EDIT</button>
                        </form>
                    </div>
        ...
        -webkit-box-shadow: 10px 14px 0px 0px rgba(0,0,0,0.53);
        -moz-box-shadow: 10px 14px 0px 0px rgba(0,0,0,0.53); background-color:var(--bs-light-bg-subtle);color:var(--bs-light-text);border:1px solid var(--bs-light-border-subtle);">
                        <h3 class="ms-3">Data Karyawan</h3>
