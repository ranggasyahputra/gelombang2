        ...
        <div class="side-navbar" style="background-color: var(--bs-dark-bg-subtle);">
            <nav>
                <ul>
                    ...
                    <li class="nav-item"><a href="#" class="fw-semibold"><i class="fa-solid fa-plus"></i>&nbsp;&nbsp;Additional Info</a></li>
...
-moz-box-shadow: 10px 14px 0px 0px rgba(0,0,0,0.53);background-color:var(--bs-light-bg-subtle);color:var(--bs-light-text);border:1px solid var(--bs-light-border-subtle);">
                        <h3 class="ms-3">Belum ClockIn</h3>
                        <table class="table table-striped ms-2 me-2 mt-2" style="width:90%">
                            ...
                            include("../connection.php");
                            $user_id = $_SESSION["user_id"];
                            $sql = "SELECT * FROM users JOIN absensi ON users.user_id = absensi.user_id WHERE tgl != '$tgl' GROUP BY nama_lengkap ORDER BY users.user_id";
                            $result = $db->query($sql);
                            $no = 1;
...
-webkit-box-shadow: 10px 14px 0px 0px rgba(0,0,0,0.53);
-moz-box-shadow: 10px 14px 0px 0px rgba(0,0,0,0.53); background-color:var(--bs-light-bg-subtle);color:var(--bs-light-text);border:1px solid var(--bs-light-border-subtle);">
                        <h3 class="ms-3">Belum ClockOut</h3>
                        <!-- TABEL DATA KARYAWAN -->
                        <table class="table table-striped ms-2 me-2 mt-2" style="width:90%">
                            <tr class="tr-header table-dark">
                                <th>No</th>
                            ...
                            include("../connection.php");
                            $user_id = $_SESSION["user_id"];
                            $sql = "SELECT * FROM users JOIN absensi ON users.user_id = absensi.user_id WHERE tgl = '$tgl' AND jam_keluar = NULL AND jam_masuk IS NOT NULL GROUP BY nama_lengkap ORDER BY users.user_id";
                            $result = $db->query($sql);
                            $no = 1;
                            while ($data = $result_belum_clockout->fetch_assoc()) {
                                echo "<tr>";
                                echo "<td>" . $no++ . "</td>";
                                echo "<td>" . $data["nama_lengkap"] . "</td>";
                                echo "<td>" . $data["user_id"] . "</td>";
        ...
        -moz-box-shadow: 10px 14px 0px 0px rgba(0,0,0,0.53); background-color:var(--bs-light-bg-subtle);color:var(--bs-light-text);border:1px solid var(--bs-light-border-subtle);">
                        ...
                        <table class="table table-striped ms-2 me-2 mt-2" style="width:90%">
                            ...
                            <?php
                            while ($data = $result_is_home->fetch_assoc()) {
                                echo "<tr>";
                                echo "<td>" . $no++ . "</td>";
                                echo "<td>" . $data["nama_lengkap"] . "</td>";
                                echo "<td>" . $data["user_id"] . "</td>";
